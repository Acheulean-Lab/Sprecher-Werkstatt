...
export function importSessionJson(text: string): Session {
  ...
  if (!s.id || !s.name) throw new Error('Invalid session');
  return s as Session;
}
...
export function exportCsv(session: Session) {
  ...
  for (const m of session.measurements) {
    const header = `${m.position}_db`.replace(/[^\w]+/g, '_');
...
export function exportFrd(session: Session, m: Measurement) {
  const lines: string[] = [];
  ...
  downloadBlob(blob, `${safeFilename(session.name)}_${safeFilename(m.position)}.frd`);
}
